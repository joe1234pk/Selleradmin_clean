        ...
        foreach ($matches as $match) {
            ...
            if (is_object($class)) {
                if (method_exists($class, $attr)) {
                    ...
                    $replace[$match[0]] = '%EMPTY_VAR1%';
                }
            } else {
                $replace[$match[0]] = '%EMPTY_VAR2%';
            }
        }
        ...
        foreach ($matches as $match) {        		
            ...
            foreach ($data as $pos => $row) {
                ...
                foreach ($row as $key => $val) {
                    ...
                    $temp[$this->l_delim.$key.$this->r_delim] = ( $val!='' ? $val : 'null');//author:dmh
                }
    ...
    protected function _replace_unparsed($template)
    {
        ...
        if ( ! empty($unparsed)) {
            foreach ($unparsed as $u) {
                $template = str_ireplace($u[0], '%EMPTY_VAR3%', $template);
            }
        }
        ...
        if ( ! empty($unparsed)) {
            foreach ($unparsed as $u) {
                if ( ! in_array($u[0], array($this->l_delim.'else'.$this->r_delim, $this->l_delim.'break'.$this->r_delim, $this->l_delim.'default'.$this->r_delim))) {
                    $template = str_ireplace($u[0], '%EMPTY_VAR4%', $template);
                }
            }
        }
